...
//
// Auth uses the RESEND_API_KEY environment variable. Every email is blind-copied
// to DEFAULT_NOTIFY_EMAIL; set DOWNLOAD_NOTIFY_EMAIL to send the copy elsewhere.
...
// Who receives a blind copy of each download email.
const DEFAULT_NOTIFY_EMAIL = "happihood@me.com";
// Where the "Order now" button in the email points. The edition number is appended
// as a hash fragment, e.g. https://trashtalk.cloud/#d=150
...
export default async (req) => {
  ...
  // The edition link is decided here rather than taken from body.orderBaseUrl, so
  // a visitor on a cached copy of the page still gets the current destination in
  // their email instead of whichever one their stale bundle would have sent.
  const orderUrl = edition
    ? `${ORDER_BASE_URL.split("#")[0]}#d=${encodeURIComponent(edition)}`
    : orderFallbackUrl;
...
<body style="margin:0;padding:0;background:#f4eef7;">
  ...
  // A blind copy so the shop sees every design that goes out. Skipped when the
  // customer happens to be that same address, since Resend would then send twice.
  const notifyEmail = (Netlify.env.get("DOWNLOAD_NOTIFY_EMAIL") || DEFAULT_NOTIFY_EMAIL).trim();
